...
- Both files must follow their specifications — DepsContract and AdapterLib, located in [Specs](/docs/Specs/doc.md).
...
6. Write a second implementation of the same contract as another adapter lib when a test or a different deployment needs it, and compose it into a `Deps` of your own following [ComposeDeps](/docs/ComposeDeps/doc.md). `standard.New` always binds **every** lib under `adapters/libs/`, so two libs filling the same field would have the last one in listing order win — keep the alternative outside `adapters/libs/` or in its own `availables/<name>/`.
