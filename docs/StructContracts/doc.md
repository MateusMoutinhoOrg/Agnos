...
Composing a whole assembly of your own is [ComposeDeps](/docs/ComposeDeps/doc.md).
...
- Every binder in `sandbox/binds/` must assign **every** field of its `api` struct; adding an action means visiting both files in the same commit — see [AddAction](/docs/AddAction/doc.md).
