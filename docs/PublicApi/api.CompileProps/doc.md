...
Describes one cross-compile run for [`api.Actions.Compile`](/docs/PublicApi/api.Actions/doc.md#compile): the directory holding the project and the targets to build. `Compile` runs [`Build`](/docs/PublicApi/api.Actions/doc.md#build) with `RuntimeGo` first, creates `release/`, then runs `go build -o release/<file> ./cmd/main` once per target with `CGO_ENABLED=0` and the target's `GOOS`/`GOARCH`.
