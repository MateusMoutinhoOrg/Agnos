...
Inside Agnos, no action calls it directly: every action goes through [SmartIO](/docs/SmartIO/doc.md), which wraps it with a transaction and a root and calls it only at the boundary. Installed by the `iodeps` dep.
