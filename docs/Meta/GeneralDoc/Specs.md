...
## Description
Defines the baseline shape every `.md` file in this project must have, whatever its kind. It is the common ground the other documentation specifications build on: a README, a tutorial, or a reference page must satisfy **this** spec plus the one for its own kind, located through [Specs.md](/docs/Specs.md).
...
- Always use **relative paths** (e.g. `../../docs/PublicApi.md`), never absolute filesystem paths.
...
- When a change affects documentation, update every impacted file in the **same commit** — see [RULES.md](/docs/RULES.md).
...
> **Note**: For a concrete example, refer to [sample.md](/docs/Meta/GeneralDoc/sample.md).
