...
- Completeness is **not** checked by the compiler. A field no factory's return value is assigned into stays nil and panics on the first call, so keeping `New` complete is the author's job. See [StructContracts.md](/docs/StructContracts.md).
