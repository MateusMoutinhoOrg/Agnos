...
### Rules
- `deps.go` must declare a single `Deps` **struct** — the one contract every adapter fills. Contracts are structs, never interfaces; see [StructContracts.md](/docs/StructContracts.md).
...
- `deps.go` must not import anything from `adapters/`, `libraryExamples/`, `sandbox/internal/`, or `sandbox` (the entry point) — the contract stays free of implementations.
