...
## Description
Covers declaring a scaffolded project's command surface from a Go program — the `AddCommand`, `AddFlag`, `AddArg`, `SetCommand` and `Remove*` actions behind the commands of [ShapeCommands](/docs/ShapeCommands/doc.md). Initializing the library is [LibInitialization](/docs/LibInitialization/doc.md); the props structs are detailed in [api.FieldProps](/docs/PublicApi/api.FieldProps/doc.md) and [api.CommandProps](/docs/PublicApi/api.CommandProps/doc.md).
