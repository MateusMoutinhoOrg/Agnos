...
## Description
Covers exposing an action on `agnos`'s own command line. Agnos is itself a project of the shape it generates, so a command here is the same three files as everywhere else — `entries.yaml`, generated `entries.go`, hand-written `handler.go` under `sandbox/internal/commands/<name>/` — and is declared with the same commands a user runs, pointed at this checkout. The logic the command calls is [AddAction](/docs/AddAction/doc.md); the generic handler rules are [WriteCommandHandler](/docs/WriteCommandHandler/doc.md).

### Rules
- Run the **bootstrap binary**, never an installed `agnos`, against this checkout — see [BootstrapAgnos](/docs/BootstrapAgnos/doc.md).
...
- Adding a command requires updating [Commands](/docs/Commands/doc.md) in the same commit; `help` regenerates itself.
...
5. Regenerate with the bootstrap binary — `entries.go`, `climain.go` and `help` catch up — and try the command:
   ...
   ```
6. Add the command's row to the table of [Commands](/docs/Commands/doc.md), in its category, with every flag and argument.
7. Finish with the bootstrap loop of [BootstrapAgnos](/docs/BootstrapAgnos/doc.md) so the installed `agnos` carries the new command.
