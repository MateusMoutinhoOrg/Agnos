...
8. Add `--quiet` when the confirmation lines are noise — in a loop, for instance:
   ...
   ```
9. Check what a command did from a script by reading its exit code: `0` ran, `1` the command line was wrong, `2` the command failed. See [Commands.md](/docs/CliUsage/References/Commands.md#exit-codes):
   ```bash
