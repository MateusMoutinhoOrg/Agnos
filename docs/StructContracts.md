...
Binding a method into a field would work in Go, but the project does not do it: one shape for filling struct contracts means one place to look for completeness — the `New` at the bottom of the file — on both sides of the wall. The rule is binding; see [RULES.md](/docs/RULES.md#factory-pattern) and the [Factories](/docs/Meta/Factories/Specs.md) specification.
...
Every field whose signature already matches is copied with one line. With interfaces, the same wiring needs a bridging type declaring every method, even the ones that pass straight through. The `bootstrap/` tree is the worked example — see [Structure.md](/docs/Structure.md#bootstrap).
