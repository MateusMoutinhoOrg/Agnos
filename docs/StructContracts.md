...
## Description
Explains why every contract in this project — `deps.Deps` and everything in `sandbox/contracts/api` — is a **struct of function fields** instead of an interface, what that buys, and what it costs. Second stage of the Development learning path, after [SandboxIsolation.md](/docs/SandboxIsolation.md); [HandleDependencies.md](/docs/HandleDependencies.md) shows the mechanic in motion.
...
Binding a method into a field would work in Go, but the project forbids it: one shape for filling contracts means one place to check completeness — the `New` at the bottom of the file — on both sides of the wall. The rule is binding; see [RULES.md](/docs/RULES.md#factory-pattern) and the [Factories](/docs/Meta/Factories/Specs.md) specification. A field that is not a function has its factory return a value rather than a closure.
...
- Adding a contract field means visiting every adapter — see [HandleDependencies.md](/docs/HandleDependencies.md#add-a-dependency).
