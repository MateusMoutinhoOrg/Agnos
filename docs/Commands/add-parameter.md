...
```bash
agnos add-parameter --route <route> [--type <type>] [--font <font>...] [--required] [--default <default>] [--trigger <trigger>] [--trigger-type <trigger-type>] [--description <description>] [--example <example>...] [--position <position>] [--path <path>] [--quiet] <name>
```
...
| `--type` | string | `string` | the value type: string, number, boolean, datetime or string-array (defaults to string) |
| `--font` | string, repeatable |  | where the value is read from, in order: query or header (repeatable; defaults to query) |
