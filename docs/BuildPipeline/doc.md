...
Template vars: `Module`, `Name` (the project being generated), `GeneratorName` (the cli running the build — `agnos`; every command agnos owns is spelled with it, never with `Name`), `Version`, `ProjectName`, `ConfigDir`, `StructureConfFile`, `HasDeps`, `HasCli`, `HasServer`, `HasAssets`, `Themes`, plus the collector outputs. The two parsing collectors read the sources as they are on disk at collect time, so a doc comment added to a *generated* contract file shows up on the next build. Native template funcs: `render "<path>"` (read a project file through the transaction, render it with the same vars, nestable) and `copy "<path>"` (verbatim). Missing target = hard error. `README.md` = `render ConfigDir/docs/ReadmeHeader.md` + the `DocIndex` sections + a link to `LICENSE`. It is the single entry point to the docs: there is no index file between it and a doc.
