...
Where Verb's fields return plain values, Keep's return further api structs, so this copy restates the whole tree and the adapter's `KeepLibFactory` wraps each such field in a closure that converts what it returns — nothing of the embedded library ever reaches the sandbox. See [DepsMechanic.md](/docs/DepsMechanic.md#injecting-a-whole-library).
