...
Where Verb's fields return plain values, Keep's return further api structs, so this copy restates the whole tree and the adapter's `KeepLibFactory` wraps each such field in a closure that converts what it returns — nothing of the embedded library ever reaches the sandbox. See [HandleDependencies.md](/docs/HandleDependencies.md#injecting-a-whole-library).

The adapter also chooses the backend Keep itself runs on: `standard` wires Keep's filesystem adapter. See [Adapters.md](/docs/Adapters.md).
