...
The command-line interface, run inside the sandbox. It reads the commands and flags of the given argument vector through the injected [`deps.Deps.VerbLib`](/docs/verbdeps.Lib.md) parser, calls the other fields of [`api.Lib`](/docs/api.Lib.md) to do the work, prints every result and error through [`deps.Deps.Printf`](/docs/deps.Deps.md), and returns the process exit code. Every command and flag it understands is listed in [Cli.md](/docs/Cli.md).
...
`args` must be the same vector the adapter wired `Deps.VerbLib` over: the parser owns the reading, and `args` is what `Sandboxmain` checks for an empty command line. [`standard.New`](/docs/standard.New.md) and `cmd/main` both take it from `os.Args[1:]`, so they agree by construction.
