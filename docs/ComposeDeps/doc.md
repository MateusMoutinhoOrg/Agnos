...
## Description
Covers building the `Deps` a scaffolded project's sandbox runs on from parts other than `standard.New()` — replacing one field for a test, or assembling a different mix of adapter libs for another deployment. Writing a new capability is [AddAdapterLib](/docs/AddAdapterLib/doc.md); the trade-offs of struct contracts, which make this a plain assignment, are [StructContracts](/docs/StructContracts/doc.md).
