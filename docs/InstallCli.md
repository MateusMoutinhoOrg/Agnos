...
## Description
Covers installing the `agnos` command-line interface globally, so it runs from any directory and survives a terminal restart. To use it once installed, follow [UseCli.md](/docs/UseCli.md); to consume the same behavior as a Go library instead, follow [LibInitialization.md](/docs/LibInitialization.md).

### Rules
- Go 1.22 or newer must be installed and available on your `PATH` (`go version` must print a version).
- The binary is built from [cmd/main](/cmd/main/), so `go install` names it `main` — the install commands below rename it to `agnos`.
...
1. Copy and paste this entire block into a terminal (bash, zsh, or any POSIX shell):
   ...
   go install github.com/MateusMoutinhoOrg/Agnos-Cli/cmd/main@v0.0.3 \
     ...
     && agnos version
   ```
2. If `agnos version` printed the version, the install is complete. Open a **new terminal** to make the PATH change available everywhere.
...
5. If `agnos version` says "command not found" after installing, `GOPATH/bin` is not on your PATH. Run `go env GOPATH` to find the directory, and add its `bin/` subdirectory to your shell profile manually:
   ```bash
   ...
   [Environment]::SetEnvironmentVariable('PATH',
     [Environment]::GetEnvironmentVariable('PATH','User') + ";$gobin", 'User')
   ```
6. If `mv` (or `Move-Item`) fails because `agnos` already exists from a previous install, delete the old binary first and re-run the install command.
