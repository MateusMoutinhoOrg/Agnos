...
## Description
Covers converting a library that already exists into this project's dependency-injected structure. To start a new library from scratch, follow [ForkTemplate.md](/docs/ForkTemplate.md) instead.
...
3. Rewrite `sandbox/contracts/deps/deps.go` with the OS-bound and third-party calls the library must receive as dependencies, one function field each, following [AddDependency.md](/docs/AddDependency.md).
4. Rewrite `sandbox/contracts/api/api.go` with the `Lib` struct and one struct per type the library hands back, following [AddLibFunction.md](/docs/AddLibFunction.md) and [AddLibObject.md](/docs/AddLibObject.md).
5. Rewrite `adapters/standard/standard.go` so the default adapter fills every field of that contract with the library's current behavior, following [AddAdapter.md](/docs/AddAdapter.md).
6. Rewrite the existing library code into `sandbox/internal/`: move each source file in, turn each public function into a `<Field>Factory(l *api.Lib)` that returns a closure for the matching api field, assign every factory's return value from the package's `New` constructor, and replace **every** OS-bound or third-party call with a call through `l.Deps.<Field>(...)`, following [AddLibFunction.md](/docs/AddLibFunction.md) and [AddLibObject.md](/docs/AddLibObject.md). Do not keep the code in its original packages, leave methods on internal types, or leave direct calls in place.
7. Create any additional adapter in `adapters/`, following [AddAdapter.md](/docs/AddAdapter.md).
8. Create the samples demonstrating the converted entry points: the Go programs in `libraryExamples/`, following [AddSample.md](/docs/AddSample.md), and the shell scripts in `cliExamples/`, following [AddCliExample.md](/docs/AddCliExample.md).
9. Create the detail pages in `docs/References/PublicApi/` and rewrite `docs/References/PublicApi.md`, following [ExposePublicApi.md](/docs/ExposePublicApi.md).
10. Delete every **[Delete](/docs/TemplateFileActions.md#delete)** file carried over from the template, plus the pre-existing code the converted lib replaced. For `.md` files, follow [DeleteDocument.md](/docs/DeleteDocument.md).
11. Rewrite `docs/References/Structure.md` to describe the library's actual layout.
12. Create the tutorials specific to the converted library — one page per workflow its maintainers will repeat (e.g. adding a domain object, extending a feature, releasing) — following [AddDocument.md](/docs/AddDocument.md) and the [TutorialDocs specification](/docs/Meta/TutorialDocs/Specs.md). The template tutorials copied in step 2 cover the structure only; they do not document the library's own use cases.
13. Create any reference page the library needs beyond the public API — following [AddDocument.md](/docs/AddDocument.md) and the [ReferenceDocs specification](/docs/Meta/ReferenceDocs/Specs.md).
14. Rewrite the `README.md`: overview, both quick starts, badges, Doc Index, and the two Examples sections.
