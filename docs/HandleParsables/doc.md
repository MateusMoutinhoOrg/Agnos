...
- The struct's operations are **function fields** assigned by `bind_methods.go`, not Go methods — the same struct-contract shape as everything else; see [StructContracts](/docs/StructContracts/doc.md).
...
7. If `agnos start` should write the file, add its template to the `start` asset group following [HandleAssetGroups](/docs/HandleAssetGroups/doc.md), and register the file in [Structure](/docs/Structure/doc.md#agnosconfig).
