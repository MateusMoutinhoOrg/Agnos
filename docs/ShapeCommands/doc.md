...
## Description
Covers declaring the whole surface of a command — its flags, positional arguments, aliases, help text and visibility — through `agnos` commands, so `entries.yaml` is never edited by hand. Creating the command in the first place is step 5 of [ScaffoldProject](/docs/ScaffoldProject/doc.md); what every key means, and how it becomes a Go field, is [EntriesYaml](/docs/EntriesYaml/doc.md); writing the handler that receives the values is [WriteCommandHandler](/docs/WriteCommandHandler/doc.md).
...
- The `help` command belongs to `agnos`: `add-command help`, `remove-command help`, and hand edits to its `handler.go` are refused or overwritten — see [CommandDispatch](/docs/CommandDispatch/doc.md#the-help-command).
