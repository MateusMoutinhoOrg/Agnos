...
## Description
Defines the required shape of an installable **dep** — a directory `assets/deplist/<dep>/` that `agnos dep-install <dep>` renders into a project, plus its entry in `assets/depsversion.yaml` when it pulls an external module. Adding one is [HandleDeplist](/docs/HandleDeplist/doc.md); the shipped deps are listed in [DepList](/docs/DepList/doc.md).
...
- The tree under it mirrors the target project: every file sits at the path it will be written to. A dep carries at least `sandbox/deps/<dep>/<dep>.go` (a [DepsContract](/docs/Specs/DepsContract/doc.md)) and `adapters/libs/<lib>/<File>.go` (an [AdapterLib](/docs/Specs/AdapterLib/doc.md)), and any other file the capability needs where it must land (`embeddeps` carries `assets/asset.go`).
- Every file is a Go `text/template` following the [AssetTemplate](/docs/Specs/AssetTemplate/doc.md) specification; the target module path is `{{.Module}}`, and that is the only variable a dep uses.
...
> **Note**: For a concrete example, refer to [sample](/docs/Specs/Dep/sample.md), which lists the tree of one shipped dep.
