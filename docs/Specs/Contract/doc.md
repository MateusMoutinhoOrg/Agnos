...
- Every field is mirrored by exactly one assignment in `sandbox/binds/<same file>.go` — the Binder specification — and every new field or props struct gets a sub-doc under `docs/PublicApi/`.
...
> **Note**: For a concrete example, refer to [sample.go](/docs/Specs/Contract/sample.go).
