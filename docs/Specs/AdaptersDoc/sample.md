...
## Description
Lists every adapter lib and assembly shipped under `adapters/`, and when to use each one. To build a new lib, follow [AddAdapterLib](/docs/AddAdapterLib/doc.md).
...
This sample's `frozen` assembly is hypothetical, so its factory is unlinked: an entry links to its `docs/PublicApi/<pkg>.<Symbol>/doc.md` sub-doc only when that page is real.
