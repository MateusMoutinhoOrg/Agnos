...
## Description
Defines the required shape of a **parsable** — a package under `sandbox/internal/parsables/<name>conf/` that parses one configuration file into a struct, exposes operations on it as function fields, and renders it back. Adding one is [HandleParsables](/docs/HandleParsables/doc.md).
...
> **Note**: For a concrete example, refer to [sample.go](/docs/Specs/Parsable/sample.go), which shows the five files in one listing.
