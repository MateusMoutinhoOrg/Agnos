...
- After changing a template or a collector, regenerate this tree with the bootstrap binary — see [BootstrapAgnos](/docs/BootstrapAgnos/doc.md). `assets/` is never compiled, so a broken template only fails there.
...
3. Bootstrap and check the file appears in this tree and in a scratch project:
   ...
   ```
4. Register the file in [GeneratedFiles](/docs/GeneratedFiles/doc.md) and in the group's row of [Structure](/docs/Structure/doc.md#assets).
...
8. Document the new key in the variables table of [BuildPipeline](/docs/BuildPipeline/doc.md#template-variables).
