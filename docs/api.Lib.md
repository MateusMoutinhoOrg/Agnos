...
The library entry point, returned by [`lib.New`](/docs/lib.New.md). It is a financial tracker: categories hold spend and received transactions, and every record is persisted through the schema database injected as [`deps.Deps.KeepLib`](/docs/keepdeps.Lib.md). It is exposed as a struct of function fields: `lib.New` stores the injected [`deps.Deps`](/docs/deps.Deps.md) in `Deps`, then runs the factories in `sandbox/internal/lib/`, each of which fills one function field with a closure reading `Deps` at call time. The same deps are propagated into every [`Category`](/docs/api.Category.md) and [`Transaction`](/docs/api.Transaction.md) the lib creates. Calling a field reads exactly like calling a method — `l.AddCategory("groceries")`. See [StructContracts.md](/docs/StructContracts.md).
...
| [`AddSpend func(category string, description string, amount int64) (Transaction, bool)`](/docs/api.AddSpend.md) | Records money leaving the budget under an existing category. |
| [`AddReceived func(category string, description string, amount int64) (Transaction, bool)`](/docs/api.AddReceived.md) | Records money entering the budget under an existing category. |
| [`ListTransactions func() []Transaction`](/docs/api.ListTransactions.md) | Returns every transaction of every category. |
| [`Balance func() int64`](/docs/api.Balance.md) | Sums the signed amounts of every stored transaction. |
