...
## Description
Explains `sandbox/internal/smartio` — the transactional filesystem every action reads and writes through, rooted at the project directory named by `--path`. It is what lets an action compose several changes and flush them once, and what makes `--path` an absolute boundary. The actions using it are described in [BuildPipeline.md](/docs/References/BuildPipeline.md); writing an action against it is [AddAction.md](/docs/Tutorials/AddAction.md).
