...
- Factories reach dependencies only through the object's `Deps` field, calling them as function fields: `o.Deps.<Field>(...)`. `sandbox/` is a closed sandbox: an object package must never import `adapters/`, `libraryExamples/`, a third-party module, or an OS-bound standard-library package (`os`, `net`, `syscall`, …). See [SandboxIsolation.md](/docs/Explanations/SandboxIsolation.md).
