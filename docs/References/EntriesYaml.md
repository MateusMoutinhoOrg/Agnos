...
## Description
Every key of `sandbox/internal/commands/<name>/entries.yaml` — the hand-written declaration of one command that `agnos build` turns into a typed `Entries` struct and a dispatch arm — and what each key makes the generated code do. The file is grown from the command line by [ShapeCommands.md](/docs/Tutorials/ShapeCommands.md); how the dispatch consumes it is explained in [CommandDispatch.md](/docs/References/CommandDispatch.md); its required shape is the CommandEntries specification in [Specs.md](/docs/References/Specs.md).
