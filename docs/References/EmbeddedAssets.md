...
## Description
How the library reads text, and later images and item templates, out of files instead of holding them in Go string constants: the assets under [`/assets/`](/docs/References/Structure.md#assets), the [`embeddeps.Lib`](/docs/References/PublicApi/embeddeps.Lib.md) contract they arrive through, and what a consumer can do with them.

---

## Why Assets Are a Dependency

Reading a file is an OS-bound effect, and compiling one into a binary needs Go's `//go:embed` directive — a package-level, filesystem-bound mechanic. The sandbox may do neither, so assets follow the same route every other effect follows: a field on the [`Deps`](/docs/References/PublicApi/deps.Deps.md) contract, filled by an adapter outside the sandbox. See [SandboxIsolation.md](/docs/References/SandboxIsolation.md).
...
The adapter serves the whole asset tree compiled into the binary, and every path is resolved against its root — [`standard.New`](/docs/References/PublicApi/standard.New.md) takes no configuration for it:
...
Only [`Sandboxmain`](/docs/References/PublicApi/api.Sandboxmain.md) reads them. A program calling `AddCategory`, `AddSpend`, `ListTransactions` or `Balance` directly never touches an asset, so a hand-built `deps.Deps` may leave `EmbedDeps` zero — with the usual cost of a struct contract: calling into the interface afterwards panics on the nil `ReadFile` rather than failing to compile. See [StructContracts.md](/docs/References/StructContracts.md#what-it-costs).
