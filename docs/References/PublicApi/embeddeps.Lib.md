...
`RenderTemplate` is how every generated file is produced: `utils.RenderGroup` lists a group with `ListFilesRecursively` and renders each file as a Go `text/template` with one `vars` map — see [BuildPipeline.md](/docs/References/BuildPipeline.md#asset-groups-in-order). Installed by the `embeddeps` dep, which also carries `assets/asset.go`.
