...
Assets are how the library holds no display text of its own. Every word the command-line interface prints — the usage screen, the version, and each message — is a file under [`/assets/`](/docs/References/Structure.md#assets) that `Sandboxmain` reads by path. Changing the interface's wording, or translating it, is editing those files; no Go changes and no recompilation of the sandbox are involved.
