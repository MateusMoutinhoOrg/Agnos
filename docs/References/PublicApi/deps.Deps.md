...
`VerbLib`, `KeepLib` and `EmbedDeps` are the exceptions to "every field is a function": the dependency is itself a library built with this pattern, so it arrives as one plain struct field — [`verbdeps.Lib`](/docs/References/PublicApi/verbdeps.Lib.md), [`keepdeps.Lib`](/docs/References/PublicApi/keepdeps.Lib.md), [`embeddeps.Lib`](/docs/References/PublicApi/embeddeps.Lib.md) — with no getter around it. The sandbox never imports the embedded Verb or Keep libraries, nor Go's `embed` machinery; it declares a copy of each api in `sandbox/contracts/deps/verbdeps/`, `keepdeps/` and `embeddeps/`, and the adapter, which lives outside the sandbox, initializes the real library and assigns its fields onto that copy.
