...
`VerbLib` is the exception to "every field is a function": the dependency is itself a library built with this pattern, so it arrives as one plain struct field — [`verbdeps.Lib`](/docs/References/PublicApi/verbdeps.Lib.md) — with no getter around it. The sandbox never imports the embedded Verb library; it declares a copy of Verb's api in `sandbox/contracts/deps/verbdeps/`, and the adapter, which lives outside the sandbox, initializes the real library and assigns its fields onto that copy.
...
func main() {
	...
	d := agnosdeps.Deps{
		...
		// VerbLib is left zero: nothing here parses arguments. Fill it —
		// as the adapters do — before anything calls through it.
	}
