...
The command-line interface of the sandbox as a single function. `CliMain` takes an argument vector — what `cmd/main` passes as `os.Args[1:]` — reads the verb, parses the remaining arguments into the matching command's typed `Entries`, rejects anything unknown or invalid, calls the command's `CommandHandler`, and returns the process exit code. There is no command registry to consult: the function is **generated** by `agnos build` from every `sandbox/internal/commands/<name>/entries.yaml` — see [CommandDispatch.md](/docs/References/CommandDispatch.md). `sandbox/api/cli.go` itself belongs to the `cli` asset group and is regenerated too.
