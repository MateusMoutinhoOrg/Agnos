...
## Description
Defines the required shape of a **collector** — a function in `sandbox/internal/actions/build/collect_*.go` that turns one directory listing of the target project into the data a `{{range}}` in an asset template iterates over. Collectors are why a generated file's contents are predictable from a listing; see [BuildPipeline.md](/docs/References/BuildPipeline.md#collectors). Adding one is [HandleAssetGroups.md](/docs/Tutorials/HandleAssetGroups.md#add-a-variable-through-a-collector).
...
- The result is added to the `vars` map in `build_internal.go` under the key the templates range over, and that key is documented in the variables table of [BuildPipeline.md](/docs/References/BuildPipeline.md#template-variables).
...
> **Note**: For a concrete example, refer to [sample.go](/docs/References/Specs/Collector/sample.go).
