...
## Description
Defines the required shape of an adapter in `adapters/<name>/<name>.go`. This spec describes **how an adapter must be built**, not which concrete dependencies it fills in — those come from the [Deps contract](/docs/References/Specs/Deps/Specs.md). An adapter fills its contract with **factories**, the same way `sandbox/internal/` fills an api struct: the [Factories](/docs/References/Specs/Factories/Specs.md) specification applies on top of this one.
...
- An adapter may import `sandbox/contracts/deps` but must never import `sandbox/internal/`. It may import `sandbox` (the entry point) only when it needs to initialize an embedded library to inject — see [StructContracts.md](/docs/References/StructContracts.md).
