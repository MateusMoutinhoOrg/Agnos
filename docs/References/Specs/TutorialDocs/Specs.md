...
## Description
Defines the required shape of a **Tutorial** page — any page under `docs/Tutorials/`, which guides a workflow. A tutorial covers a single goal (e.g. `ScaffoldProject.md`, `AddAction.md`), built from actionable numbered steps.
...
6. **`## Full Code`** *(required when the workflow assembles a program across steps)*: the complete resulting code in a single fenced block, ready to copy and run — see [WriteCommandHandler.md](/docs/Tutorials/WriteCommandHandler.md) for a concrete example.
