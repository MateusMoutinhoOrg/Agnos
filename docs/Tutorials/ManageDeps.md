...
## Description
Covers the dependency-injection layer of a scaffolded project: turning it on and off with `deps-init` / `deps-purge`, and adding or removing capabilities with `dep-install` / `dep-remove`. What each installable dep brings is listed in [DepList.md](/docs/References/DepList.md); why the sandbox receives everything through `Deps` is explained in [SandboxIsolation.md](/docs/References/SandboxIsolation.md); writing a capability of your own instead of installing one is [AddAdapterLib.md](/docs/Tutorials/AddAdapterLib.md).
