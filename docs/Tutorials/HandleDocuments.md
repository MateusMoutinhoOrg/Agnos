...
- A page can belong to one or more **themes** — `CliUsage`, `GeneratedProject`, `LibUsage`, `Development` — and its themes are expressed by the indexes that list it, never by the page's location.
...
2. Find every reference to the old path:
   ```bash
   grep -rn "OldName.md" --include="*.md" . --exclude-dir=old
   ```
...
1. Find every reference to the document:
   ```bash
   grep -rn "DocName.md" --include="*.md" . --exclude-dir=old
   ```
