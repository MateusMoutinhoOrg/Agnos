...
## Description
Covers filling in `sandbox/internal/commands/<name>/handler.go` — the one file of a command you write by hand — in a project scaffolded by `agnos`. The declaration next to it is grown from the command line by [ShapeCommands.md](/docs/Tutorials/ShapeCommands.md); how the generated dispatch turns that declaration into the `Entries` the handler receives is [CommandDispatch.md](/docs/References/CommandDispatch.md); an effect the handler needs and no installed dep provides is [AddAdapterLib.md](/docs/Tutorials/AddAdapterLib.md).
...
- The handler lives inside the closed sandbox: it may import `sandbox/api`, `sandbox/deps` and anything under `sandbox/internal`, and nothing outside `sandbox/` — no `os`, no `fmt`, no third-party module. `verify` rejects the import; see [SandboxIsolation.md](/docs/References/SandboxIsolation.md).
- Every effect goes through a `deps` field named after its `sandbox/deps/<x>/` directory: `deps.Std`, `deps.Iodeps`, `deps.Requestdeps`, … — see [PublicApi.md](/docs/References/PublicApi.md#dependency-contracts).
