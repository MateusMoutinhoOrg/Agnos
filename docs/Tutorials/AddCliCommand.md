...
## Description
Covers adding a command or a flag to the command-line interface — the dispatch behind [`api.Lib.Sandboxmain`](/docs/References/PublicApi/api.Sandboxmain.md), which lives in [sandbox/internal/cli/](/sandbox/internal/cli/). Adding the *library* function a command calls is a separate goal, covered by [AddLibFunction.md](/docs/Tutorials/AddLibFunction.md); a command that needs a new OS-bound effect needs [AddDependency.md](/docs/Tutorials/AddDependency.md) first.

### Rules
- The interface is inside the closed sandbox: it may not import `adapters/`, `cmd/`, a third-party module, or an OS-bound standard-library package. It reads the command line through `l.Deps.VerbLib` and prints through `l.Deps.Printf`, and nothing else. See [SandboxIsolation.md](/docs/Explanations/SandboxIsolation.md).
...
- Adding a command requires updating the `Usage` screen in `sandbox/internal/cli/cli.go` and [Cli.md](/docs/References/Cli.md) in the same commit.
...
5. Build and try it:
   ...
   ```
6. Add the command to the Commands table of [Cli.md](/docs/References/Cli.md), and any flag to its Flags table.
7. Demonstrate it in a script under [cliExamples/](/cliExamples/) when it is worth showing, following [AddCliExample.md](/docs/Tutorials/AddCliExample.md).
