...
4. Write the constructor's factory in [sandbox/internal/lib/](../../sandbox/internal/lib/), propagating `l.Deps` into the new object:
   ...
   ```
5. Assign `NewBudgetFactory`'s return value in the lib package's `New` constructor, as described in [AddLibFunction.md](/docs/AddLibFunction.md).
6. If a factory needs a dependency that is not yet in the contract, add it following [AddDependency.md](/docs/AddDependency.md).
7. Expose the object, its constructor, and its fields following [ExposePublicApi.md](/docs/ExposePublicApi.md).
8. Register the new directory and file in [Structure.md](/docs/Structure.md).
9. If the object needs a runnable demonstration, add one following [AddSample.md](/docs/AddSample.md).
