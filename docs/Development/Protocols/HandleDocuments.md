...
- Every page lives in a **theme** directory — `CliUsage/`, `LibUsage/`, `Development/`, `Templating/` — under `Protocols/` if it is a workflow, under `References/` if it is a lookup or an explanation.
...
## Add a Document
1. Identify the theme the document belongs to, and whether it is a **Protocol** (a workflow) or a **Reference** (a lookup or an explanation).
...
3. Create the `.md` file in `docs/<Theme>/<Protocols|References>/` with a descriptive, topic-based name (e.g., `HandleSamples.md`, `PublicApi.md`).
...
6. Add a row to the theme's `Index.md`, in the `Protocols` or the `References` table.
7. Register the file in [Structure.md](/docs/Development/References/Structure.md).

---

## Rename or Move a Document
1. Rename or move the `.md` file, keeping a descriptive, topic-based name and landing in a `Protocols/` or `References/` directory of a theme.
