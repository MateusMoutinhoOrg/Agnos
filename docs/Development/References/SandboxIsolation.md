...
## Description
Explains what "closed sandbox" means in practice: the library in `sandbox/` reaches nothing outside itself — no adapter, no third-party module, no OS-bound standard-library package — so everything it can do is exactly what the injected `Deps` allows. First stage of the Development learning path; [StructContracts.md](/docs/Development/References/StructContracts.md) explains how the contracts crossing this wall are shaped.
