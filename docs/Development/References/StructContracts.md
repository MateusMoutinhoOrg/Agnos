...
Binding a method into a field would work in Go, but the project forbids it: one shape for filling contracts means one place to check completeness — the `New` at the bottom of the file — on both sides of the wall. The rule is binding; see [RULES.md](/docs/Development/References/RULES.md#factory-pattern) and the [Factories](/docs/Development/References/Specs/Factories/Specs.md) specification. A field that is not a function has its factory return a value rather than a closure.
...
Every field whose signature matches is one line; an interface would need a bridging type declaring every method. The `bootstrap/` tree is the worked example, explained end to end in [Bootstrap.md](/docs/LibUsage/References/Bootstrap.md) — see also [Structure.md](/docs/Development/References/Structure.md#bootstrap).
