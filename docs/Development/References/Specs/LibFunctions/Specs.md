...
- `sandbox/` is a closed sandbox: a factory must never import `adapters/`, `examples/libraryExamples/`, a third-party module, or an OS-bound standard-library package (`os`, `net`, `syscall`, …). See [SandboxIsolation.md](/docs/Development/References/SandboxIsolation.md).
- A closure returning a library object returns that object's **api struct**, built by the object package's `New` constructor — see the [LibObjects](/docs/Development/References/Specs/LibObjects/Specs.md) specification.
...
- Factories and the fields they fill must have doc comments, and the fields must be listed in [PublicApi.md](/docs/LibUsage/References/PublicApi.md).
