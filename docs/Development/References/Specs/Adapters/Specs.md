...
## Description
Defines the required shape of an adapter in `adapters/<name>/<name>.go`. This spec describes **how an adapter must be built**, not which concrete dependencies it fills in — those come from the [Deps contract](/docs/Development/References/Specs/Deps/Specs.md). An adapter fills its contract with **factories**, the same way `sandbox/internal/` fills an api struct: the [Factories](/docs/Development/References/Specs/Factories/Specs.md) specification applies on top of this one.
...
- An adapter may import `sandbox/contracts/deps` but must never import `sandbox/internal/`. It may import `sandbox` (the entry point) only when it needs to initialize an embedded library to inject — see [StructContracts.md](/docs/Development/References/StructContracts.md).
- The adapter is the **opinionated** layer, and it lives outside the sandbox: all concrete choices (OS-bound stdlib, third-party libs, config) live here and nowhere else. See [SandboxIsolation.md](/docs/Development/References/SandboxIsolation.md).
