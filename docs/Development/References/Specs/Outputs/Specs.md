...
### Rules
- `api.go` must declare one **struct** per object the library hands back, including the `Lib` entry point returned by `lib.New`. Contracts are structs, never interfaces; see [StructContracts.md](/docs/Development/References/StructContracts.md).
...
- `api.go` declares **types only** — never a function body. Every implementation lives in `sandbox/internal/`; see the [LibFunctions](/docs/Development/References/Specs/LibFunctions/Specs.md) specification.
...
- Exported structs must have a doc comment and be listed in [PublicApi.md](/docs/LibUsage/References/PublicApi.md).
