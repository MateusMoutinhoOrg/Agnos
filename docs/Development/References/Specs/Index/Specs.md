...
- The index lists **every** `.md` file of its theme: one row per page in `Protocols/`, one row per page in `References/`. No orphans. Pages already indexed by another page — the API details under `PublicApi/`, the specifications under `Specs/` — are covered through that page's row.
- A page is listed by the index of **its own** theme. A page from another theme is linked in prose, never given a row.
- Each row is `[FileName.md](/docs/<Theme>/<Protocols|References>/<FileName>.md)` plus a one-line description, 50–100 characters, saying what the reader gets.
...
- Creating, renaming, moving, or deleting a `.md` file requires updating its theme index in the same commit — see [HandleDocuments.md](/docs/Development/Protocols/HandleDocuments.md).
...
4. **`## Protocols`**: a table of every workflow page of the theme, with `Doc` and `Description` columns.
5. **`---`**: horizontal rule.
