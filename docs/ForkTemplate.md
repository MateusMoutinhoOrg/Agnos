...
4. Rewrite [sandbox/contracts/deps/deps.go](../../sandbox/contracts/deps/deps.go) with the dependencies the new library requires, following [AddDependency.md](/docs/AddDependency.md).
5. Rewrite [sandbox/contracts/api/api.go](../../sandbox/contracts/api/api.go) with the `Lib` struct and one struct per object the new library hands back, following [AddLibFunction.md](/docs/AddLibFunction.md) and [AddLibObject.md](/docs/AddLibObject.md).
6. Rewrite [adapters/standard/standard.go](../../adapters/standard/standard.go) so the default adapter fills every field of the new contract, following [AddAdapter.md](/docs/AddAdapter.md).
7. Create the new library logic in [sandbox/internal/](../../sandbox/internal/) — the lib's factories plus one package per object — following [AddLibFunction.md](/docs/AddLibFunction.md) and [AddLibObject.md](/docs/AddLibObject.md).
8. Create any additional adapter in [adapters/](../../adapters/), following [AddAdapter.md](/docs/AddAdapter.md).
9. Create the new samples: the Go programs in [libraryExamples/](../../libraryExamples/), following [AddSample.md](/docs/AddSample.md), and the shell scripts in [cliExamples/](../../cliExamples/), following [AddCliExample.md](/docs/AddCliExample.md).
...
11. Delete every remaining **[Delete](/docs/TemplateFileActions.md#delete)** file — the example internal logic, adapters, samples, and API pages the new library replaced. For `.md` files, follow [DeleteDocument.md](/docs/DeleteDocument.md).
12. Rewrite [docs/References/Structure.md](/docs/Structure.md) to describe the resulting layout.
13. Create the tutorials specific to the new library — one page per workflow its maintainers will repeat — following [AddDocument.md](/docs/AddDocument.md) and the [TutorialDocs specification](/docs/Meta/TutorialDocs/Specs.md). The template tutorials cover the structure only; they do not document the library's own use cases.
14. Create any reference page the library needs beyond the public API, following [AddDocument.md](/docs/AddDocument.md) and the [ReferenceDocs specification](/docs/Meta/ReferenceDocs/Specs.md).
