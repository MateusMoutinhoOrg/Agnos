...
| agnos | v0.8.2+ | every generated file — the tree cannot be maintained by hand |

This tree was rendered by `agnos v0.8.2`, so that is the floor: an older
binary rewrites it to its own older shape.

```bash
go version      # go1.25.0 or newer
agnos version  # v0.8.2 or newer
```
