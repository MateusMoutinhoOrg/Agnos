...
Creates the category `name` in the injected database, stamping it with the injected clock (`l.Deps.Now()`), and returns it as an [`api.Category`](/docs/LibUsage/References/PublicApi/api.Category.md) with the library's deps propagated in.
...
| Type | Description |
| :--- | :--- |
| [`api.Category`](/docs/LibUsage/References/PublicApi/api.Category.md) | The created or already-stored category, or the zero value on failure. |
