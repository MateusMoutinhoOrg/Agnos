...
Records money **leaving** the tracked budget under an existing category and returns the persisted record as an [`api.Transaction`](/docs/LibUsage/References/PublicApi/api.Transaction.md) of kind `Spend`, stamped with the injected clock (`l.Deps.Now()`).

It is the lib-level shorthand for `l.GetCategory(category)` followed by `Category.AddSpend(...)`: the category must already exist, so declare it with [`AddCategory`](/docs/LibUsage/References/PublicApi/api.AddCategory.md) first. `amount` is expressed in the smallest currency unit (cents) and must be **positive** — the direction of the money is the transaction's `Kind`, not the sign of its amount.
