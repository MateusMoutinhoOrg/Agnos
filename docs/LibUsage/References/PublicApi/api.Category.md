...
One bucket transactions are tracked under — "groceries", "salary" — already persisted in the injected database. `Deps` is the dependency set propagated from the parent [`api.Lib`](/docs/LibUsage/References/PublicApi/api.Lib.md); `Id`, `Name` and `CreatedAt` are plain data read from the stored record; the function fields are filled by factories in `sandbox/internal/category/`, whose closures re-read the stored record through `Deps` on every call. A `Category` is always constructed by [`AddCategory`](/docs/LibUsage/References/PublicApi/api.AddCategory.md), [`GetCategory`](/docs/LibUsage/References/PublicApi/api.GetCategory.md), or [`ListCategories`](/docs/LibUsage/References/PublicApi/api.ListCategories.md), which propagate the deps in. The factories behind it live inside the closed sandbox and are not reachable by callers.
...
| Field | Description |
| :--- | :--- |
| [`Deps deps.Deps`](/docs/LibUsage/References/PublicApi/api.Deps.md) | The dependency set propagated from the lib; read-only after construction. |
