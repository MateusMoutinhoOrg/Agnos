...
3. Grep every adapter's `New` for the new factory assignment to be sure none was missed — this is the step that replaces the compiler check:
   ...
   ```
4. Use the dependency from the library through `l.Deps.<Field>(...)`, following [AddLibFunction.md](/docs/AddLibFunction.md).
5. If the requirement changes how dependencies behave for consumers, update [DepsMechanic.md](/docs/DepsMechanic.md).
