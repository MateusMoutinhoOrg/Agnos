...
## Description
Covers adding a capability to the set `agnos dep-install` can render into a project — a directory under `assets/deplist/<dep>/` mirroring the files the dep installs, plus its pinned module in `assets/depsversion.yaml` when it pulls one. The pair every dep is made of is described in [AddAdapterLib](/docs/AddAdapterLib/doc.md); how a dep is rendered and removed is [BuildPipeline](/docs/BuildPipeline/doc.md#deps); the shape the directory must have is the Dep specification in [Specs](/docs/Specs/doc.md).
...
## Workflow
1. Write the contract and the adapter lib in this repository first, following [AddAdapterLib](/docs/AddAdapterLib/doc.md), and bootstrap so Agnos itself uses them — see [BootstrapAgnos](/docs/BootstrapAgnos/doc.md).
...
6. Rebuild the bootstrap binary — the `assets` package embeds `all:*`, so the new directory exists at runtime with no other change — and try it against a scratch project:
   ...
   ```
7. Add the dep's row to [DepList](/docs/DepList/doc.md) and, when the contract is public API, a sub-doc under `docs/PublicApi/` listed by [PublicApi](/docs/PublicApi/doc.md) — required by [HandleDocuments](/docs/HandleDocuments/doc.md).
