...
func assetPath(s *StandardAdapter, requested string) string {
	joined := path.Join(s.embedDir, requested)
	if joined == "" {
		return "."
	}
	return joined
...
// project's assets, compiled into the binary by the assets package and rooted
// at the directory this adapter was built with. It returns a value rather
// than a closure because the field is a struct — see the Factories
// specification — and each of that struct's own fields is a closure reading
// the adapter's root at call time.
func EmbedDepsFactory(s *StandardAdapter) embeddeps.Lib {
	return embeddeps.Lib{
		ReadFile: func(requested string) ([]byte, error) {
			return assets.Files.ReadFile(assetPath(s, requested))
		},
		ListFiles: func(requested string) ([]string, error) {
			return assetFiles(assetPath(s, requested), false)
		},
		ListFilesRecursively: func(requested string) ([]string, error) {
			return assetFiles(assetPath(s, requested), true)
		},
	}
}
