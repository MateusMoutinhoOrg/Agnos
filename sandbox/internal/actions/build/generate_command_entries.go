...
func GenerateCommandEntries(sandbox *api.Sandbox, io *smartio.SmartIO, commands []map[string]any) error {
	for _, command := range commands {
		...
		if err := utils.RenderTemplateToDest(sandbox, io, "templates/entries.go", command, dest); err != nil {
			return err
		}
	}
	return nil
