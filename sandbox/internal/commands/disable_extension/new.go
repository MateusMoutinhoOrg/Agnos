...
func NewCommand(sandbox *api.Sandbox) api.Command {
	...
	command.Name = "disable_extension"
	command.Identifiers = []string{"disable-extension"}
	command.Category = "Extensions"
	command.Help = "Turn one generation mechanic off"
	...
	command.Flags = []api.CommandFlag{
		{
			...
			Examples:    []string{"disable-extension --path ./my-project"},
			Default:     ".",
			HasDefault:  true,
	...
	command.Args = []api.CommandArg{
		{
			...
			Required:    true,
			Array:       false,
			Description: "the extension to turn off",
			Examples:    []string{"disable-extension doc"},
			Default:     "",
			HasDefault:  false,
		},
	}
