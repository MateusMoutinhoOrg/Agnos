...
func NewCommand(sandbox *api.Sandbox) api.Command {
	...
	command.Identifiers = []string{"remove-database"}
	command.Category = "Database System"
	command.Help = "Delete one database package whole"
	command.LongDescription = "Deletes sandbox/internal/databases/<name>/ and runs build without compiling: dropping a database may leave hand-written code calling what is gone. It refuses a package carrying a methods_custom.go, which is the project's own and which nothing in the declaration describes."
	command.Examples = []string{"remove-database app-database"}
	...
	command.Args = []api.CommandArg{
		{
			...
			Required:    true,
			Array:       false,
			Description: "the database (identifier or package name) to delete",
			Examples:    []string{"remove-database app-database"},
			Default:     "",
			HasDefault:  false,
		},
	}
