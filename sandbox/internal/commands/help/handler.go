...
var helpCommands = []helpCommand{
	...
	{
		Identifiers:     []string{"add-field"},
		Category:        "Server System",
		Description:     "Add a field to a route's route.yaml",
		LongDescription: "Declares one field on a route and runs build so entries.go and the dispatch arm pick it up. --in says where it is read from: path appends a segment (--identifier for a literal, --name for a capture), header and query declare a request field, body declares a property of the json-schema at a dotted path.",
		Examples:        []string{"add-field tenant --route create-user --in path --required", "add-field authorization --route create-user --in header --required", "add-field page --route create-user --in query --type int --default 1 --min 1", "add-field address.city --route create-user --in body --required"},
		Hidden:          false,
		Flags: []helpField{
			{Identifiers: []string{"--route"}, Description: "the route (identifier or package name) that receives the field", Examples: []string{"add-field tenant --route create-user --in path"}, Type: "string", Default: "", Required: true},
			{Identifiers: []string{"--in"}, Description: "where the field is read from: path, header, query or body", Examples: []string{"add-field authorization --route create-user --in header"}, Type: "string", Default: "query", Required: false},
			{Identifiers: []string{"--identifier"}, Description: "declare a literal path segment instead of a capture, always starting with / (--in path only)", Examples: []string{"add-field --route create-user --in path --identifier /users"}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--type"}, Description: "the value type: string, boolean, int or float (defaults to string)", Examples: []string{"add-field page --route create-user --type int"}, Type: "string", Default: "string", Required: false},
			{Identifiers: []string{"--description"}, Description: "help text shown for the field", Examples: []string{"add-field tenant --route create-user --description \"the tenant the user belongs to\""}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--example"}, Description: "an usage example for the field (repeatable)", Examples: []string{"add-field page --route create-user --example \"?page=2\""}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--default"}, Description: "the literal assigned when the field is absent (cannot be combined with --required)", Examples: []string{"add-field page --route create-user --type int --default 1"}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--required"}, Description: "answer 400 when the field is not provided (not for booleans or fields with --default)", Examples: []string{"add-field tenant --route create-user --in path --required"}, Type: "boolean", Default: "", Required: false},
			{Identifiers: []string{"--array"}, Description: "collect every occurrence into a []T field instead of a single value (query only)", Examples: []string{"add-field tag --route create-user --in query --array"}, Type: "boolean", Default: "", Required: false},
			{Identifiers: []string{"--min"}, Description: "smallest accepted value, or shortest accepted text for a body string", Examples: []string{"add-field page --route create-user --type int --min 1"}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--max"}, Description: "largest accepted value, or longest accepted text for a body string", Examples: []string{"add-field page --route create-user --type int --max 100"}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--position"}, Description: "zero-based index to insert the field at (defaults to the end)", Examples: []string{"add-field tenant --route create-user --in path --position 0"}, Type: "int", Default: "-1", Required: false},
			{Identifiers: []string{"--format"}, Description: "json-schema format for a body string: email, uuid, date-time or uri (--in body only)", Examples: []string{"add-field email --route create-user --in body --format email"}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--pattern"}, Description: "regular expression a body string must match (--in body only)", Examples: []string{"add-field zip --route create-user --in body --pattern \"^[0-9]{5}$\""}, Type: "string", Default: "", Required: false},
			{Identifiers: []string{"--path"}, Description: "the dir holding the project (defaults to the current directory)", Examples: []string{"add-field --path ./my-project"}, Type: "string", Default: ".", Required: false},
			{Identifiers: []string{"--quiet", "-q"}, Description: "Quiets the cli output", Examples: []string{}, Type: "boolean", Default: "", Required: false},
		},
		Args: []helpField{
			{Name: "name", Description: "the field name: the captured segment, the header name, the query key, or the dotted path of a body property (omitted when --identifier declares a literal path segment)", Examples: []string{"add-field tenant --route create-user --in path"}, Type: "string", Default: "", Required: false},
		},
	},
	{
	...
	{
		Identifiers:     []string{"remove-field"},
		Category:        "Server System",
		Description:     "Delete one declared field from a route",
		LongDescription: "Drops the named field from the origin --in names, the exact inverse of add-field. The build renders only: dropping a field may leave hand-written code referring to what is gone.",
		Examples:        []string{"remove-field page --route create-user --in query", "remove-field /users --route create-user --in path"},
		Hidden:          false,
		Flags: []helpField{
			{Identifiers: []string{"--route"}, Description: "the route (identifier or package name) the field is declared on", Examples: []string{"remove-field page --route create-user --in query"}, Type: "string", Default: "", Required: true},
			{Identifiers: []string{"--in"}, Description: "where the field is declared: path, header, query or body", Examples: []string{"remove-field authorization --route create-user --in header"}, Type: "string", Default: "query", Required: false},
			{Identifiers: []string{"--path"}, Description: "the dir holding the project (defaults to the current directory)", Examples: []string{"remove-field --path ./my-project"}, Type: "string", Default: ".", Required: false},
			{Identifiers: []string{"--quiet", "-q"}, Description: "Quiets the cli output", Examples: []string{}, Type: "boolean", Default: "", Required: false},
		},
		Args: []helpField{
			{Name: "name", Description: "the field to drop: its name, the identifier of a path segment, or the dotted path of a body property", Examples: []string{"remove-field page --route create-user --in query"}, Type: "string", Default: "", Required: true},
		},
	},
	{
