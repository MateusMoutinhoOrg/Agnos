...
- **`sandbox/internal/cli/`** — the command-line interface itself: the `Run(l *api.Lib, args []string) int` dispatch `SandboxmainFactory` delegates to, the paths of the text it prints, and the amount parser. Like `store/` it is neither an object nor the entry point, so no spec governs it and it declares no types and no factories. It holds **no display text**: `asset` and `message` read it from `assets/` through `l.Deps.EmbedDeps`. Adding a command means editing here, `assets/cli/usage.txt`, **and** `docs/CliUsage/References/Commands.md` — see `docs/Development/Tutorials/HandleCliCommands.md`.
- **`assets/`** — outside the sandbox; the text the library displays, compiled into the binary by `assets.go`'s `//go:embed` directives and served by the standard adapter's `embed.go`. `version.txt` is the version the interface reports, `cli/usage.txt` the help screen, `cli/messages/<name>.txt` one file per line the interface can print. The `//go:embed` directive can only reach inside its own package directory, which is why this is a Go package.
...
| add or change a CLI command or flag | `docs/Development/Tutorials/HandleCliCommands.md` | `assets/cli/usage.txt` + `docs/CliUsage/References/Commands.md` |
| add, edit, or move a file under `assets/` | `docs/Development/Tutorials/HandleAssets.md` | the `//go:embed` patterns in `assets/assets.go` + `docs/Development/References/Structure.md` |
