...
- **`sandbox/contracts/deps/deps.go`** — the `Deps` **struct**. Adding a requirement = adding a function field here. This is the contract every adapter must fill. One field is not a function: `VerbLib` is the embedded Verb argv-parser library, injected whole.
