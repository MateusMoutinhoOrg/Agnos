...
`sandbox/` is a **closed sandbox**: nothing in it may import `adapters/`, `cmd/`, `examples/libraryExamples/`, a third-party module, or an OS-bound stdlib package (`os`, `net`, `syscall`, …) — nor `embed`, nor the text it displays. Every such effect is a `Deps` field reached through `l.Deps`. This is a binding rule — see `docs/Development/References/RULES.md` and `docs/Development/References/SandboxIsolation.md`.
...
- **`cmd/main/main.go`** — outside the sandbox; the installed binary. Wires the standard adapter into the lib — `agnosadapter.New(dataPath(), assetRoot)`, with `assetRoot` the whole asset tree — calls `l.Sandboxmain(os.Args[1:])`, and `os.Exit`s with its return. The argument vector must be the same one the adapter wired `Deps.VerbLib` over. Governed by the `CliMain` spec.
...
| add or change a CLI command or flag | `docs/Development/Tutorials/HandleCliCommands.md` | `assets/usages.txt` + `docs/CliUsage/References/Commands.md` |
| add, edit, or move a file under `assets/` | `docs/Development/Tutorials/HandleAssets.md` | `docs/Development/References/Structure.md` (the `//go:embed all:*` directive in `assets/asset.go` needs no change) |
