...
What `{{.GeneratorName}} add-dep <dep>` renders into this project. A dep is named after the contract it
...
`{{.GeneratorName}} list-deps` prints the same names; `{{.GeneratorName}} remove-dep <dep>` takes one back out. Writing a
