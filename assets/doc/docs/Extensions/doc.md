...
An extension is one **generation mechanic**: a set of files `{{.GeneratorName}}` writes and
keeps up to date for you. `{{.ConfigDir}}/extensions.yaml` declares which ones are on, and it
...
| `readme` | `README.md`, built from `{{.ConfigDir}}/docs/ReadmeHeader.md` and the doc index |
...
```bash
{{.GeneratorName}} list-extensions                  # every mechanic and whether it is on
{{.GeneratorName}} enable-extension readme          # start generating README.md again
{{.GeneratorName}} disable-extension doc            # stop generating docs/
```
...
{{ if .HasCli }}
```bash
{{.GeneratorName}} cli-init                         # sandbox-cli: true, plus the deps the layer needs
{{.GeneratorName}} cli-purge                        # removes the files and writes sandbox-cli: false
```
{{ else }}
```bash
{{.GeneratorName}} deps-init                        # sandbox-deps: true
{{.GeneratorName}} deps-purge                       # removes the files and writes sandbox-deps: false
```
{{ end }}
