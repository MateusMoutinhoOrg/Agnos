...
// of sandbox.Routes; the dispatch calls New once per request, so a request's
// bound values are its own.
...
{{- end}}
	...
	route.New = func() *api.Route {
		return NewRoute(sandbox)
	}

	route.Handler = func() int {
		return RouteHandler(sandbox, route, routeio.ResponseOf(route))
	}
