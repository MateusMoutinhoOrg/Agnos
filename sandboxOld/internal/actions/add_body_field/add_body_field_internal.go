...
func AddBodyFieldInternal(sandbox *api.Sandbox, io *smartio.SmartIO, props api.RouteBodyFieldProps) error {
	...
	parent, err := walkTo(sandbox, conf.Body.Schema, parts[:len(parts)-1])
	if err != nil {
		return err
	}
	...
	schema, err := propertySchema(sandbox, props)
	if err != nil {
		return err
	}
...
// or a list of them.
func walkTo(sandbox *api.Sandbox, root *routeconf.Schema, segments []string) (*routeconf.Schema, error) {
	parent := root
...
func propertySchema(sandbox *api.Sandbox, props api.RouteBodyFieldProps) (*routeconf.Schema, error) {
	kind, err := utils.RouteSchemaKind(sandbox, props.Type)
	if err != nil {
		return nil, err
	}
	format, err := utils.RouteSchemaFormat(sandbox, props.Format)
	if err != nil {
		return nil, err
	}
	...
	if err := applyBounds(sandbox, leaf, kind, props); err != nil {
		return nil, err
	}
	if err := applyObjectKeys(sandbox, leaf, kind, props); err != nil {
		return nil, err
	}

	return applyListKeys(sandbox, leaf, props)
}
...
func applyBounds(sandbox *api.Sandbox, schema *routeconf.Schema, kind string, props api.RouteBodyFieldProps) error {
	...
	if props.ExclusiveMin != "" || props.ExclusiveMax != "" {
		...
		if props.ExclusiveMin != "" {
			value, err := utils.RouteParseBound(sandbox, kind, "exclusive-min", props.ExclusiveMin)
			if err != nil {
				return err
			}
		...
		if props.ExclusiveMax != "" {
			value, err := utils.RouteParseBound(sandbox, kind, "exclusive-max", props.ExclusiveMax)
			if err != nil {
				return err
			}
	...
	if props.Min != "" {
		value, err := utils.RouteParseBound(sandbox, kind, "min", props.Min)
		if err != nil {
			return err
		}
	...
	if props.Max != "" {
		value, err := utils.RouteParseBound(sandbox, kind, "max", props.Max)
		if err != nil {
			return err
		}
...
func applyObjectKeys(sandbox *api.Sandbox, schema *routeconf.Schema, kind string, props api.RouteBodyFieldProps) error {
...
func applyListKeys(sandbox *api.Sandbox, leaf *routeconf.Schema, props api.RouteBodyFieldProps) (*routeconf.Schema, error) {
	if !props.Array {
